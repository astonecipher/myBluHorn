...
if (! $hiddenCols["cume"]) {
$tbl .= <<<EOD
<th style="$borderBottomStr width: 40px;">Cume</th>
EOD;
$totalCols++;
}
...
if (! $hiddenCols["cpp"]) {
$tbl .= <<<EOD
<th style="$borderBottomStr width: 40px;">CPP</th>
EOD;
$totalCols++;
}
...
if (! $hiddenCols["cpm"]) {
$tbl .= <<<EOD
<th style="$borderBottomStr width: 40px;">CPM</th>
EOD;
$totalCols++;
}
...
if (! $hiddenCols["aqh"]) {
$tbl .= <<<EOD
<th style="$borderBottomStr width: 40px;">AQH</th>
EOD;
$totalCols++;
}
...
$tbl .= <<<EOD
<th style="width: {$weekWidth}; text-align: center;">$week</th>
EOD;
$weeksOnLine++;
...
$tbl .= <<<EOD
<th style="width: {$weekSkip}; text-align: center;"></th>
EOD;
...
if (! $hiddenCols["cume"]) {
$linetbl .= <<<EOD
<td style="width: 40px;">{$line["cume"]}</td>
EOD;
}

if (! $hiddenCols["cpp"]) {
$linetbl .= <<<EOD
<td style="width: 40px;">{$line["cpp"]}</td>
EOD;
}

if (! $hiddenCols["cpm"]) {
$linetbl .= <<<EOD
<td style="width: 40px;">{$line["cpm"]}</td>
EOD;
}

if (! $hiddenCols["aqh"]) {
$linetbl .= <<<EOD
<td style="width: 40px;">{$line["aqhRating"]}{$sheet["demo"][1]["description"]}</td>
EOD;
}
...
$linetbl .= <<<EOD
<td style="width: {$weekWidth}; text-align: center;">{$spotCount}</td>
EOD;
...
$linetbl .= <<<EOD
<td style="width: {$weekSkip}; text-align: center;"></td>
EOD;
...
<th style="width: 90px; $borderBottomStr white-space: nowrap;" nowrap></th>
...
$tbl .= <<<EOD
<th style="width: {$weekWidth}; text-align: center;">{$weekSummary[$week]["totalSpots"]}
EOD;
