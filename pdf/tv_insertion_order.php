...
<th width="65" style="width: 45px; $borderBottomStr">Call</th>
<th width="120" style="width: 120px; $borderBottomStr" nowrap>Program Title</th>
<th style="$borderBottomStr">Rate/Total</th>
<th style="$borderBottomStr">CPP</th>
EOD;

if (!$worksheetCols["cpm"]) {
$tbl .= <<<EOD
...
EOD;
	...
	if ($totalLinesOnPage + $headerLineCount > 22){
		if ($printedLines > 0) {	
...
<td style="">{$line["rate"]} {$line["total"]}</td>
...
/*
$attachment = $pdf->Output('order.pdf', 'E');


		$email = new \mg($this->db);
		
//		$email->from("orders@bluhorn.com");
		$email->from("orders@bluhorn.com");
		$email->cc("wbenwick@bluhorn.com");
		$email->to("wbenwick@gmail.com");
		$email->subject("Station Order");
//		$email->text("A new order has been attached.");
//		$email->html("<html><strong>Please see attachment.</strong></html>");
		$email->contents($attachment);
