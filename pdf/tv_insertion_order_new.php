...
for ($offset=0; $offset<count($weekNames); $offset += $maxWeeks) {
	...
	if ($totalLinesOnPage + $headerLineCount > 40){
		if ($printedLines > 0) {	
...
error_log("New Table TotalLinesOnPage: " . $totalLinesOnPage  . " header lines: " . $headerLineCount);
...
<th style="width: 60px; $borderBottomStr white-space: nowrap;" nowrap>Days</th>
...
<th style="width: 60px; $borderBottomStr">Rate/Total</th>
EOD;
...
$tbl .= <<<EOD
<th style="width: 45px; $borderBottomStr text-align: center;"> Spots</th>
</tr>
</thead>
EOD;
	...
	if ($totalLinesOnPage + $headerLineCount > 46){
		if ($printedLines > 0) {	
			//$pdf->writeHTML("--Page Break-- $totalLinesOnPage $headerLineCount $printedLines", true, false, false, false, '');
...
<td style="width: 60px; white-space: nowrap;" nowrap>$daysStr</td>
...
<td style="width: 60px;">{$line["rate"]} <br /> {$line["totalSpend"]}</td>
EOD;

error_log("Cols CPP: " . $hiddenCols["cpp"]);
...
error_log("Cols CPM: " . $hiddenCols["cpm"]);
...
$linetbl .= <<<EOD
<td style="width: 45px; text-align: center;">{$totalSpotsPerLine}</td>
</tr>
EOD;
if ($line["comments"]) {
$linetbl .= <<<EOD
...
<td style="" colspan="18"> <strong><i style="color: #f00;">{$line["comments"]}</i></strong></td>
<td></td>
</tr>
</tbody>
EOD;
		...
		if (($line["totalSpots"] > 0) or ($line["comments"] != "")) {
			...
			error_log("Adding Line TotalLinesOnPage: " . $totalLinesOnPage . " header lines: " . $headerLineCount);
...
<th style="width: 70px; $borderBottomStr white-space: nowrap;" nowrap></th>
...
EOD;
	...
	//$pdf->SetXY(1,30,true);
if (($totalSpots>0) or ($totalLinesOnPage > 0)) {
	$pdf->writeHTML($tbl, true, false, false, false, '');
}
