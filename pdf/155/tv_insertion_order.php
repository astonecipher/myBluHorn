...
$tbl = "";
	...
	if ($offset > 0) {
		if ($totalLinesOnPage + $headerLineCount > 20){
			if ($printedLines > 0) {	
...
<th style="width: 65px; $borderBottomStr">Call</th>
<th style="width: 120px; $borderBottomStr" nowrap>Program Title</th>
...
$tbl .= <<<EOD
<th style="width: {$weekCellSize}; text-align: center;">$week</th>
EOD;
$weeksOnLine++;
...
$tbl .= <<<EOD
<th style="width: {$weekCellSize}; text-align: center;"></th>
EOD;
...
EOD;
	...
	if ($totalLinesOnPage + $headerLineCount > 26){
		if ($printedLines > 0) {	
			//$pdf->writeHTML("--Page Break-- $totalLinesOnPage $headerLineCount $printedLines", true, false, false, false, '');
...
<td style="white-space: nowrap;" nowrap>{$line["daypart"]}/{$line["seconds"]}</td>
<td style="width: 65px;">{$line["station"]}</td>
<td style="width: 120px; white-space: nowrap;" nowrap>{$line["programName"]}</td>
...
$linetbl .= <<<EOD
<td style="width: {$weekCellSize}; text-align: center;">{$wsWeeks[$displayedLine][$weekCounter]}</td>
EOD;
...
$linetbl .= <<<EOD
<td style="width: {$weekCellSize}; text-align: center;"></td>
EOD;
...
<td style="" colspan="13"> <strong><i style="color: #f00;">{$line["comments"]}</i></strong></td>
<td></td>
</tr>
</tbody>
EOD;
...
<th style="width: 65px; $borderBottomStr"></th>
<th style="width: 120px; $borderBottomStr" nowrap></th>
<th style="$borderBottomStr text-align: center;"><strong>Spots:</strong></th>
EOD;
...
$tbl .= <<<EOD
<th style="width: {$weekCellSize}; text-align: center;">{$weekSummary[$week]["totalSpots"]}</th>
EOD;
...
$tbl .= <<<EOD
<th style="$borderBottomStr text-align: center;"> $totalSpotsPerTable</th>
</tr>
</tfoot>
EOD;
...
if ($lineCounter < 1) {
	...
	$pdf->writeHTML(summaryTable($lines, $weekSummary, $months), true, false, false, false, '');
...
if (($orderInfo["comments"] != "") or ($orderInfo["traffic"] != "")) {
...
function summaryTable($lines, $weeks, $months) {

$summary = <<<EOD
<strong>Station Summary</strong><br><table cellspacing="0" cellpadding="1" border="1" style="margin-right: 10px; width: 500px;">
	<thead>
    	<tr>
