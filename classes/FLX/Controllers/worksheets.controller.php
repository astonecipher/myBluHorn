...
class worksheets extends \controller
{  
	...
	private $auth;
	private $vars = array("activeSideBar"=>array("worksheets"=>"active"));
	private $lists;
	...
	private $fields = array("campaignID"=>"campaignID", "worksheetName"=>"name", "marketType"=>"typeID", "markets"=>"marketID", "agencyID"=>"agencyID", "useBroadcastWeeks"=>"useBroadcastWeeks", "commission"=>"commission", "isAutoComm"=>"isAutoComm","autoPopulate"=>"isAutoPopulated", "grpGoal"=>"grpGoal");
...
//	  }
	...
	if ($this->auth->validate($this->userID)) {
	 	  ...
	 	  $this->view = "worksheets";			

		  $this->vars["active"] = "worksheets";		
		  $this->vars["listType"] = ucfirst($params[1]);		
		  ...
		  $worksheets = new \BH\worksheet($this->db, $this->userID, $this->agencyID);
		  
		  $this->vars["worksheets"] = $worksheets->getAllWorksheets($this->agencyID, FALSE);
	...
	if ($this->auth->validate($this->userID)) {
		  ...
		  $this->vars["worksheets"] = $worksheets->getAllWorksheets($this->agencyID, "isPending");
	...
	if ($this->auth->validate($this->userID)) {
		  ...
		  $this->vars["worksheets"] = $worksheets->getAllWorksheets($this->agencyID, "isActive");
	...
	public function archived($params) {
	...
	if ($this->auth->validate($this->userID)) {
		  ...
		  $this->vars["worksheets"] = $worksheets->getAllWorksheets($this->agencyID, "isArchived");
	...
	public function ajax($params) {
	 ...
	 if ($this->auth->validate($this->userID)) {
		  ...
		  if ($params[2] == "save") {
			  ...
			  $result = $worksheet->updateSortByWorksheetID($worksheetID, $sorting);
			  ...
			  if ($result) {
			  		$response["message"] = "Sorting Saved!";
			  		$this->vars["response"] = json_encode($response);
				  	return true;
			  }
			  else {
			  		$response["message"] = "<span style='color:#f00'>Error Saving Sort!</span>";
			  		$response["error"] = true;
			  		$this->vars["response"] = json_encode($response);		
				  	return true;
			  }
			  ...
			  if ($worksheetID > 0) {
			  		...
			  		if ($_POST["autoPopulate"]) {
				  		
				  		$inventory = new \BH\inventory($this->db, $this->agencyID, $this->userID);
	
				  		$vendors = $_POST["vendors"];
				  		$demographics = $_POST["demographic"];
				  		
				  		$type = $worksheet->type($worksheetID);
				  		if ($type["type"] == "Cable") {
					  		$inventory->autoPopulateCableByVendor($worksheetID, $vendors, $demographics);
					  	}
					  	else if ($type["type"] == "TV") {
						  	$inventory->autoPopulateCableByVendor($worksheetID, $vendors, $demographics);
						}
					  	else if ($type["type"] == "Radio") {
						  	$inventory->autoPopulateRadioByVendor($worksheetID, $vendors, $demographics);
						}						
				  	}
	...
	//				  error_log("Data:" . print_r($_POST["changes"], true));
			  ...
			  if ($params[3] == "update") {
				  ...
				  foreach ($_POST["data"] as $data) {
					  ...
					  if ($lineNumber < $_POST["maxLineNumber"]) {
