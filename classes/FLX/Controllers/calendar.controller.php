...
class calendar
{
                            ...
                            if ($endDateTimeStr == "") {
                                ...
                                if ($eventDetails == "") {
                                    ...
                                    if ($eventDescription == "") {
                                        ...
                                        if (strtotime($startDateTimeStr) > strtotime($endDateTimeStr)) {
                                            $this->vars["alertError"] = true;
                ...
                if ($locationID) {
                    ...
                    if (! $locationName) {
                        ...
                        if (($repeats["frequency"] == "Daily") or ($repeats["frequency"] == "Every Other Day")) {
                            ...
                            if (($repeats["frequency"] == "Weekly") or ($repeats["frequency"] == "Every Other Week")) {
                                $repeats["weekDays"] = date('w', strtotime($startDateTimeStr));
                                $repeats["monthDays"] = "";
                            } else 
                                if (($repeats["frequency"] == "Monthly")) {
                                    $repeats["weekDays"] = "";
                                    $repeats["monthDays"] = date('d', strtotime($startDateTimeStr));
                                }
                ...
                if ($eventParams["sTitle"] == "") {
                    ...
                    if ($eventParams["tStart"] == "") {
                        ...
                        if ($eventParams["tEnd"] == "") {
                            ...
                            if ($eventParams["sDetails"] == "") {
                                $this->vars["alertError"] = true;
                                $this->vars["errorMsg"] = "Please provide details about the event.";
                            } 

                            else 
                                if ($eventParams["sDescription"] == "") {
                                    $this->vars["alertError"] = true;
                                    $this->vars["errorMsg"] = "Please provide a description about this event.";
                                } else 
                                    if (strtotime($eventParams["tStart"]) > strtotime($eventParams["tEnd"])) {
                                        $this->vars["alertError"] = true;
                                        $this->vars["errorMsg"] = "Please correct your Start and Ending times.";
                                    } else {
    ...
    public function js($params)
    {
        ...
        $dates = $calendar->repeatingDates("0", "", array(
            "15"
        ), "12", "", "2013-12-15", "2014-12-31");
                    ...
                    if ($params[4] == "zebra") {
                        ...
                        if ($params[4] == "standard") {
                            ...
                            if ($params[4] == "event") {
                                ...
                                if ($params[4] == "addEvent") {
                                    $this->view = "CAL_LARGE_ADDEVENT_BODY";
                                } else 
                                    if ($params[4] == "nextWeek") {
...
//         error_log($results);
        ...
        $event = $this->loadFullCalendarArray($results);
    ...
    public function loadFullCalendarArray($params)
    {
        if ( is_array($params)) {
            foreach ($params as $result) {
                // error_log(print_r($result));
                $bgColor = $result['bgColor'] == null ? '#489dc1' : $result['bgColor'];
                $fontColor = $result['fontColor'] == null ? '#fff' : $result['fontColor'];
