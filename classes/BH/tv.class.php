...
//		$timeOfDay = explode("-",$params["timePeriod"],2);
//		$startTime = $timeOfDay[0];
//		$endTime = $timeOfDay[1];
		
//		$params["timePeriodStd"] = date("H:i", strtotime($startTime)) . date("H:i", strtotime($endTime));
...
*/
	...
	public function getWorksheetLines($worksheetID) {
		...
		$r=$this->db->query("select BH_WORKSHEET_TV_LINES.*, left(BH_WORKSHEET_TV_LINES.timePeriod,5) as fromTime, right(BH_WORKSHEET_TV_LINES.timePeriod,5) as endTime, BH_VENDORS.name as vendorName from BH_WORKSHEET_TV_LINES left join BH_VENDORS on (BH_WORKSHEET_TV_LINES.vendorID = BH_VENDORS.id) where BH_WORKSHEET_TV_LINES.worksheetID = $worksheetIDStr and BH_WORKSHEET_TV_LINES.agencyID = \"$this->agencyID\" $showDeletedStr $orderStr");
//		$r=$this->db->query("select BH_WORKSHEET_TV_LINES.*, left(BH_WORKSHEET_TV_LINES.timePeriod,5) as fromTime, right(BH_WORKSHEET_TV_LINES.timePeriod,5) as endTime, BH_VENDORS.name as vendorName from BH_WORKSHEET_TV_LINES left join BH_VENDORS on (BH_WORKSHEET_TV_LINES.vendorID = BH_VENDORS.id) where BH_WORKSHEET_TV_LINES.worksheetID = $worksheetIDStr and BH_WORKSHEET_TV_LINES.agencyID = \"$this->agencyID\" $showDeletedStr order by BH_WORKSHEET_TV_LINES.worksheetLine asc");
	...
	public function getWorksheetLinesUnsorted($worksheetID) {
		$worksheetIDStr = $this->db->quote($worksheetID);
		
		$showDeletedStr = "and BH_WORKSHEET_TV_LINES.isDeleted is FALSE";
		
		$r=$this->db->query("select BH_WORKSHEET_TV_LINES.*, left(BH_WORKSHEET_TV_LINES.timePeriod,5) as fromTime, right(BH_WORKSHEET_TV_LINES.timePeriod,5) as endTime, BH_VENDORS.name as vendorName from BH_WORKSHEET_TV_LINES left join BH_VENDORS on (BH_WORKSHEET_TV_LINES.vendorID = BH_VENDORS.id) where BH_WORKSHEET_TV_LINES.worksheetID = $worksheetIDStr and BH_WORKSHEET_TV_LINES.agencyID = \"$this->agencyID\" $showDeletedStr order by worksheetLine asc");
//		$r=$this->db->query("select BH_WORKSHEET_TV_LINES.*, left(BH_WORKSHEET_TV_LINES.timePeriod,5) as fromTime, right(BH_WORKSHEET_TV_LINES.timePeriod,5) as endTime, BH_VENDORS.name as vendorName from BH_WORKSHEET_TV_LINES left join BH_VENDORS on (BH_WORKSHEET_TV_LINES.vendorID = BH_VENDORS.id) where BH_WORKSHEET_TV_LINES.worksheetID = $worksheetIDStr and BH_WORKSHEET_TV_LINES.agencyID = \"$this->agencyID\" $showDeletedStr order by BH_WORKSHEET_TV_LINES.worksheetLine asc");
	...
	public function setActiveWeeks($worksheetID, $startWeek, $endWeek) {
		...
		$this->db->query("update BH_WORKSHEET_TV_WEEKS set isActive = TRUE where worksheetID = $worksheetIDStr and weekNumber between $startWeekStr and $endWeekStr");
		$this->db->query("update BH_WORKSHEET_TV_WEEKS set isActive = FALSE where worksheetID = $worksheetIDStr and weekNumber not between $startWeekStr and $endWeekStr");
	...
	public function getWorksheetWeeks($worksheetID) {
		...
		$r=$this->db->query("select BH_WORKSHEET_TV_WEEKS.* from BH_WORKSHEET_TV_WEEKS where BH_WORKSHEET_TV_WEEKS.worksheetID = $worksheetIDStr $showDeletedStr  and isActive is TRUE order by BH_WORKSHEET_TV_WEEKS.worksheetLine asc,BH_WORKSHEET_TV_WEEKS.weekNumber asc");
	...
	public function getWorksheetWeeksByLine($worksheetID, $lineNumber) {
		...
		$r=$this->db->query("select BH_WORKSHEET_TV_WEEKS.* from BH_WORKSHEET_TV_WEEKS where BH_WORKSHEET_TV_WEEKS.worksheetID = $worksheetIDStr and worksheetLine = $lineNumberStr and isActive is TRUE $showDeletedStr order by BH_WORKSHEET_TV_WEEKS.worksheetLine asc,BH_WORKSHEET_TV_WEEKS.weekNumber asc");
	...
	public function autoPopulated($worksheetID, $source, $refID) {
		...
		$r=$this->db->query("select * from BH_WORKSHEET_TV_LINES where worksheetID = $worksheetIDStr and source = $sourceStr and refID = $refIDStr");
...
//				 error_log("Line Spend: " . $lineSpend);
				 ...
				 $this->db->update("BH_WORKSHEET_TV_LINES", "id", $line["id"], array("totalSpots"=>$lineSpots, "totalSpend"=>$lineSpend));
				 
			 }
			 
			 $worksheetIDStr = $this->db->quote($worksheetID);
			 
			 $r=$this->db->query("select sum(totalSpend) as totalSpend, sum(totalSpots) as totalSpots from BH_WORKSHEET_TV_LINES where BH_WORKSHEET_TV_LINES.worksheetID = $worksheetIDStr and BH_WORKSHEET_TV_LINES.isDeleted is FALSE");
		     $results=$r->fetch(\PDO::FETCH_ASSOC);
		
			 error_log("Total Spots And Spend: " . $results["totalSpots"] . " and " . $results["totalSpend"]);
