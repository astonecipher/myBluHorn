...
{  
	...
	public function getMaxLinesByWorksheetID($worksheetID) {
		...
		$r=$this->db->query("select max(worksheetLine) as maxLine from BH_WORKSHEET_RADIO_LINES where worksheetID = $worksheetIDStr and agencyID = '$this->agencyID' ");
	...
	public function getLineByID($worksheetLineID) {
		...
		$r=$this->db->query("select worksheetLine from BH_WORKSHEET_RADIO_LINES where id = $worksheetLineIDStr and agencyID = '$this->agencyID' ");
	...
	public function updateCellByWorksheetIDandLine($worksheetID, $line, $cell = array()) {
		...
		$id = $this->db->upsert("BH_WORKSHEET_RADIO_LINES", $params);
	...
	public function copyLinesByWorksheetID($worksheetID) {
		...
		if ($this->db->beginTransaction()) {
			...
			$r=$this->db->query("select BH_WORKSHEET_RADIO_LINES.*, BH_VENDORS.name as vendorName from BH_WORKSHEET_RADIO_LINES left join BH_VENDORS on (BH_WORKSHEET_RADIO_LINES.vendorID = BH_VENDORS.id) where BH_WORKSHEET_RADIO_LINES.worksheetID = $worksheetIDStr and BH_WORKSHEET_RADIO_LINES.agencyID = \"$this->agencyID\" and BH_WORKSHEET_RADIO_LINES.isCopy is TRUE and BH_WORKSHEET_RADIO_LINES.isDeleted is FALSE order by BH_WORKSHEET_RADIO_LINES.worksheetLine asc");
			...
			if ($lines) {
				...
				foreach ($lines as $line) {
					...
					$id = $this->db->upsert("BH_WORKSHEET_RADIO_LINES", $newLine);
					error_log("Copying Lines Upsert: $id = " . $this->db->lastQuery());

					
					if ($id>0) {			

						$copyResult = $this->db->update("BH_WORKSHEET_RADIO_LINES", "id", $line["id"], array("isCopy"=>"0"));
						error_log("Lines Copied: " . $linesCopied);
								
						$originalWorksheetLine = $line["worksheetLine"];		
						$r=$this->db->query("select BH_WORKSHEET_RADIO_WEEKS.* from BH_WORKSHEET_RADIO_WEEKS where BH_WORKSHEET_RADIO_WEEKS.worksheetID = $worksheetIDStr and worksheetLine = $originalWorksheetLine order by BH_WORKSHEET_RADIO_WEEKS.weekNumber asc");
	...
	public function updateWeekByWorksheetIDandLine($worksheetID, $line, $week = array()) {
		...
		$id = $this->db->upsert("BH_WORKSHEET_RADIO_WEEKS", $params);
	...
	public function removeLineByWorksheetID($worksheetID, $line) {
		...
		if ($this->db->beginTransaction()) {
			...
			$weeks = $this->db->updateWhere("BH_WORKSHEET_RADIO_WEEKS", array("worksheetID"=>$params["worksheetID"],"worksheetLine"=>$params["worksheetLine"]), array("isDeleted"=>"1"));
//			$weeks = $this->db->delete("BH_WORKSHEET_RADIO_WEEKS", "worksheetID", $params["worksheetID"], array("worksheetLine"=>$params["worksheetLine"]));
...
/*
			...
			$this->db->exec("update BH_WORKSHEET_RADIO_WEEKS set worksheetLine=worksheetLine-1 where worksheetLine > $lineNumber and worksheetID = $worksheetIDStr and agencyID = $agencyIDStr ");

			error_log("Update Week($line): $weeks - " . $this->db->lastQuery());

			$this->db->exec("update BH_WORKSHEET_RADIO_LINES set worksheetLine=worksheetLine-1 where worksheetLine > $lineNumber and worksheetID = $worksheetIDStr and agencyID = $agencyIDStr ");
...
*/
	...
	public function getWorksheetLines($worksheetID) {
		...
		$showDeletedStr = "and BH_WORKSHEET_RADIO_LINES.isDeleted is FALSE";
		...
		$r=$this->db->query("select BH_WORKSHEET_RADIO_LINES.*, left(BH_WORKSHEET_RADIO_LINES.timePeriod,5) as fromTime, right(BH_WORKSHEET_RADIO_LINES.timePeriod,5) as endTime, BH_VENDORS.name as vendorName from BH_WORKSHEET_RADIO_LINES left join BH_VENDORS on (BH_WORKSHEET_RADIO_LINES.vendorID = BH_VENDORS.id) where BH_WORKSHEET_RADIO_LINES.worksheetID = $worksheetIDStr and BH_WORKSHEET_RADIO_LINES.agencyID = \"$this->agencyID\" $showDeletedStr $orderStr");
	...
	public function setActiveWeeks($worksheetID, $startWeek, $endWeek) {
		...
		$this->db->query("update BH_WORKSHEET_RADIO_WEEKS set isActive = TRUE where worksheetID = $worksheetIDStr and weekNumber between $startWeekStr and $endWeekStr");
		$this->db->query("update BH_WORKSHEET_RADIO_WEEKS set isActive = FALSE where worksheetID = $worksheetIDStr and weekNumber not between $startWeekStr and $endWeekStr");
	...
	public function getWorksheetWeeks($worksheetID) {
		...
		$showDeletedStr = "and BH_WORKSHEET_RADIO_WEEKS.isDeleted is FALSE";
		
		$r=$this->db->query("select BH_WORKSHEET_RADIO_WEEKS.* from BH_WORKSHEET_RADIO_WEEKS where BH_WORKSHEET_RADIO_WEEKS.worksheetID = $worksheetIDStr $showDeletedStr and isActive is TRUE order by BH_WORKSHEET_RADIO_WEEKS.worksheetLine asc,BH_WORKSHEET_RADIO_WEEKS.weekNumber asc");
	...
	public function getWorksheetWeeksByLine($worksheetID, $lineNumber) {
		...
		$r=$this->db->query("select BH_WORKSHEET_RADIO_WEEKS.* from BH_WORKSHEET_RADIO_WEEKS where BH_WORKSHEET_RADIO_WEEKS.worksheetID = $worksheetIDStr and worksheetLine = $lineNumberStr and isActive is TRUE $showDeletedStr order by BH_WORKSHEET_RADIO_WEEKS.worksheetLine asc,BH_WORKSHEET_RADIO_WEEKS.weekNumber asc");
	...
	public function totalSpotsAndSpend($worksheetID) {
		 ...
		 if ($wrksht["id"] == $worksheetID) {
			 ...
			 foreach ($lines as $line) {
				 ...
				 $this->db->update("BH_WORKSHEET_RADIO_LINES", "id", $line["id"], array("totalSpots"=>$lineSpots, "totalSpend"=>$lineSpend));
				 
			 }
			 
			 $worksheetIDStr = $this->db->quote($worksheetID);
			 
			 $r=$this->db->query("select sum(totalSpend) as totalSpend, sum(totalSpots) as totalSpots from BH_WORKSHEET_RADIO_LINES where BH_WORKSHEET_RADIO_LINES.worksheetID = $worksheetIDStr and BH_WORKSHEET_RADIO_LINES.isDeleted is FALSE");
		     $results=$r->fetch(\PDO::FETCH_ASSOC);
