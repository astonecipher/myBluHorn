...
/**
 * BLUHORN OUTDOOR WORKSHEET CLASS
 *  
 * @author Wes Benwick
...
class outdoor extends worksheet
{  
	...
	public function getMaxLinesByWorksheetID($worksheetID) {
		...
		$r=$this->db->query("select max(worksheetLine) as maxLine from BH_WORKSHEET_OUTDOOR_LINES where worksheetID = $worksheetIDStr and agencyID = '$this->agencyID' ");
	...
	public function getLineByID($worksheetLineID) {
		...
		$r=$this->db->query("select worksheetLine from BH_WORKSHEET_OUTDOOR_LINES where id = $worksheetLineIDStr and agencyID = '$this->agencyID' ");
	...
	public function updateCellByWorksheetIDandLine($worksheetID, $line, $cell = array()) {
		...
		$id = $this->db->upsert("BH_WORKSHEET_OUTDOOR_LINES", $params);
	...
	public function copyLinesByWorksheetID($worksheetID) {
		...
		if ($this->db->beginTransaction()) {
			...
			$r=$this->db->query("select BH_WORKSHEET_OUTDOOR_LINES.*, BH_VENDORS.name as vendorName from BH_WORKSHEET_OUTDOOR_LINES left join BH_VENDORS on (BH_WORKSHEET_OUTDOOR_LINES.vendorID = BH_VENDORS.id) where BH_WORKSHEET_OUTDOOR_LINES.worksheetID = $worksheetIDStr and BH_WORKSHEET_OUTDOOR_LINES.agencyID = \"$this->agencyID\" and BH_WORKSHEET_OUTDOOR_LINES.isCopy is TRUE and BH_WORKSHEET_OUTDOOR_LINES.isDeleted is FALSE order by BH_WORKSHEET_OUTDOOR_LINES.worksheetLine asc");
			...
			if ($lines) {
				...
				foreach ($lines as $line) {
					...
					$id = $this->db->upsert("BH_WORKSHEET_OUTDOOR_LINES", $newLine);
					error_log("Copying Lines Upsert: $id = " . $this->db->lastQuery());

					
					if ($id>0) {			

						$copyResult = $this->db->update("BH_WORKSHEET_OUTDOOR_LINES", "id", $line["id"], array("isCopy"=>"0"));
...
/*
			...
			$this->db->exec("update BH_WORKSHEET_OUTDOOR_LINES set worksheetLine=worksheetLine-1 where worksheetLine > $lineNumber and worksheetID = $worksheetIDStr and agencyID = $agencyIDStr ");
...
*/
	...
	public function getWorksheetLines($worksheetID) {
		...
		$showDeletedStr = "and BH_WORKSHEET_OUTDOOR_LINES.isDeleted is FALSE";
		...
		$r=$this->db->query("select BH_WORKSHEET_OUTDOOR_LINES.*, BH_VENDORS.name as vendorName from BH_WORKSHEET_OUTDOOR_LINES left join BH_VENDORS on (BH_WORKSHEET_OUTDOOR_LINES.vendorID = BH_VENDORS.id) where BH_WORKSHEET_OUTDOOR_LINES.worksheetID = $worksheetIDStr and BH_WORKSHEET_OUTDOOR_LINES.agencyID = \"$this->agencyID\" $showDeletedStr $orderStr");
	...
	public function totalSpotsAndSpend($worksheetID) {
		 ...
		 if ($wrksht["id"] == $worksheetID) {
			 ...
			 foreach ($lines as $line) {
				 ...
				 $lineSpots = $line["numberOfBoards"];
				 
				 // multiply spots * rate = totalspend per line
				 
				 $lineSpend = $line["totalCost"];
				 
				 error_log("Line Spend: " . $lineSpend);
				 
				 $this->db->update("BH_WORKSHEET_OUTDOOR_LINES", "id", $line["id"], array("totalSpots"=>$lineSpots, "totalSpend"=>$lineSpend));
				 
			 }
			 
			 $worksheetIDStr = $this->db->quote($worksheetID);
			 
			 $r=$this->db->query("select sum(totalSpend) as totalSpend, sum(totalSpots) as totalSpots from BH_WORKSHEET_OUTDOOR_LINES where BH_WORKSHEET_OUTDOOR_LINES.worksheetID = $worksheetIDStr and BH_WORKSHEET_OUTDOOR_LINES.isDeleted is FALSE");
		     $results=$r->fetch(\PDO::FETCH_ASSOC);
